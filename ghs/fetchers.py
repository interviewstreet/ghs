...
def fetch_oldest_contribution_year(username):
  with my_session.post('https://api.github.com/graphql', json={'query': contribution_years_query(username)}, headers=get_headers()) as result:
    request = result
...
def fetch_contributors_count(repo_owner, repo_name):
  resp = requests.get(
      f'https://api.github.com/repos/{repo_owner}/{repo_name}/contributors?per_page=1&anon=true', headers=get_headers())
...
def fetch_contribution_collection(username, start_date):
  with my_session.post('https://api.github.com/graphql', json={'query': contribution_collection_query(username, start_date)}, headers=get_headers()) as result:
    request = result
...
def fetch_total_repo_commits(repo_name, repo_owner):
  with my_session.post('https://api.github.com/graphql', json={'query': total_commit_query(repo_name, repo_owner)}, headers=get_headers()) as result:
    request = result
...
def fetch_general_stats(username):
  with my_session.post('https://api.github.com/graphql', json={'query': general_stats_query(username)}, headers=get_headers()) as result:
    request = result
